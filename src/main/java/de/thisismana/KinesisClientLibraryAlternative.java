...
/*
 * Source: https://docs.aws.amazon.com/streams/latest/dev/kcl2-standard-consumer-java-example.html
 *
 ...
 */
public class KinesisClientLibraryAlternative {

    private static final Logger log = LoggerFactory.getLogger(KinesisClientLibraryAlternative.class);
     ...
     */
    private KinesisClientLibraryAlternative(String streamName, String region) {
        this.streamName = streamName;
        this.region = Region.of(ObjectUtils.firstNonNull(region, "eu-central-1"));
    ...
    public static void main(String... args) {
        ...
        String streamName = Main.STREAM_NAME;
        ...
        new KinesisClientLibraryAlternative(streamName, region).run();
    }
                ...
                processRecordsInput.records().forEach(r -> {
                    ...
                    if (null != r.data()) {
                        try {
                            ...
                            r.data().get(arr);
                            data = Main.decompress(arr);

                        } catch (Exception ex) {
                            log.error("DECODING FAILED.", ex);
                        }
                    }
                    log.info("Processing record pk: {} -- Seq: {} -- data: {}", r.partitionKey(), r.sequenceNumber(), data);
                });
            } catch (Throwable t) {
